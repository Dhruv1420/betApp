  ...
  {bets.map((bet) => (
    <React.Fragment key={bet._id}>
      {bet.generatedNumbers
        ...
        .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()) // Sort by timestamp (latest first)
        .map((gen, index) => (
          <tr key={index} className="border-b">
            <td className="py-2 text-center px-4 border-b">{gen.generatedNumber}</td>
            <td className="py-2 text-center px-4 border-b">{gen.updatedAmount.toFixed(2)}</td>
